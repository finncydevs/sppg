<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto bg-white p-8 rounded-lg shadow-md"
         x-data="{
            rows: {{ Js::from($po->items->map(fn($i) => ['item_id' => $i->item_id, 'quantity' => $i->quantity_ordered, 'price' => (float)$i->price_per_unit])) }},
            items: {{ Js::from($items) }},

            updatePrice(index) {
                let selectedId = this.rows[index].item_id;
                let item = this.items.find(i => i.id == selectedId);
                // Update harga hanya jika item berubah
                if(item) this.rows[index].price = parseFloat(item.price);
            },
        ...
        <form action="{{ route('purchase-orders.update', $po->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700">
                                Perhatian: PO ini statusnya bukan Draft. Mengedit item mungkin akan mempengaruhi stok atau laporan penerimaan yang sudah ada.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="date" id="order_date" name="order_date" value="{{ old('order_date', $po->order_date->format('Y-m-d')) }}" required
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                </div>
            </div>
            ...
            <div class="flex items-center justify-end mt-8 pt-4 border-t gap-3">
                ...
                <button type="submit" class="text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-lg text-sm px-5 py-2.5 transition-colors">
                    <i class="fas fa-save mr-1"></i> Perbarui Pesanan
                </button>
            </div>
        </form>
    </div>
