<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md"
         x-data="{
            rows: {{ Js::from($recipe->items->map(fn($item) => ['item_id' => $item->item_id, 'quantity' => $item->quantity])) }}
         }">
        ...
        <form action="{{ route('recipes.update', $recipe->id) }}" method="POST">
            ...
            <div class="mb-6">
                <label for="instruction" class="block mb-2 text-sm font-medium text-gray-700">Instruksi Memasak</label>
                <textarea id="instruction" name="instruction" rows="4"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">{{ old('instruction', $recipe->instruction) }}</textarea>
            </div>
            ...
            <div class="flex items-center justify-end mt-8 pt-4 border-t gap-3">
                ...
                <button type="submit" class="text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-lg text-sm px-5 py-2.5 transition-colors">
                    <i class="fas fa-save mr-1"></i> Perbarui Resep
                </button>
            </div>
        </form>
    </div>
